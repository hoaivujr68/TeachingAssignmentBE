...
{
    public class CuckooTeachingRepository : ICuckooTeachingRepository
    {
        private readonly TeachingAssignmentDbContext _context;
        private readonly IClassRepository _classRepository;
        private readonly ITeacherRepository _teacherRepository;

        public CuckooTeachingRepository(TeachingAssignmentDbContext context, IClassRepository classRepository, ITeacherRepository teacherRepository)
        {
            _context = context;
        ...
        public async Task<Pagination<Data.CuckooTeachingAssignment>> GetAllAsync(QueryModel queryModel, string role)
        {
            queryModel.PageSize ??= 20;
            queryModel.CurrentPage ??= 1;

            IQueryable<Data.CuckooTeachingAssignment> query = BuildQuery(queryModel, role);
        ...
        public async Task<IEnumerable<TeacherModel>> GetAvailableTeachersForClass(string classId)
        {
            ...
            var currentAssignment = await _context.CuckooTeachingAssignments
            ...
            var teachingAssignments = await _context.CuckooTeachingAssignments
        ...
        {
            return await _context.CuckooTeachingAssignments
        ...
        public async Task<Data.CuckooTeachingAssignment> GetByIdAsync(Guid id)
        {
            return await _context.CuckooTeachingAssignments.FindAsync(id);
        }

        public async Task AddAsync(Data.CuckooTeachingAssignment teachingAssignment)
        {
            ...
            await _context.CuckooTeachingAssignments.AddAsync(teachingAssignment);
            await _context.SaveChangesAsync();
        }

        public async Task<Data.CuckooTeachingAssignment> UpdateAsync(Guid id, Data.CuckooTeachingAssignment updatedTeachingAssignment)
        {
            var existingAssignment = await _context.CuckooTeachingAssignments
            ...
            _context.CuckooTeachingAssignments.Update(existingAssignment);
            await _context.SaveChangesAsync();
        ...
        {
            var teachingAssignment = await _context.CuckooTeachingAssignments.FindAsync(id);
            if (teachingAssignment != null)
            {
                _context.CuckooTeachingAssignments.Remove(teachingAssignment);
                await _context.SaveChangesAsync();
            }
        }
        ...
        {
            IQueryable<Data.CuckooTeachingAssignment> query;

            if (role == "lanhdao" || role == "admin")
            {
                query = _context.CuckooTeachingAssignments;
            }
            else
            {
                // Lọc theo teacherCode khi role không phải lanhdao hoặc admin
                query = _context.CuckooTeachingAssignments.Where(pa => pa.TeacherCode == role);
            }
        ...
        public async Task AddRangeAsync(IEnumerable<CuckooTeachingAssignment> teachingAssignments)
        {
            await _context.CuckooTeachingAssignments.AddRangeAsync(teachingAssignments);
            await _context.SaveChangesAsync();
        }

        private IQueryable<Data.CuckooTeachingAssignment> BuildQuery(QueryModel queryModel, string role)
        {
            IQueryable<Data.CuckooTeachingAssignment> query;
            ...
            {
                // Lọc theo teacherCode khi role không phải lanhdao hoặc admin
                query = _context.CuckooTeachingAssignments.Where(p => p.TeacherCode == role);
            }

            var predicate = PredicateBuilder.New<Data.CuckooTeachingAssignment>();
